...
Run it on a schedule so a dependency that turns malicious after install still
gets caught:
...
jobs:
  recheck:
    ...
    steps:
      ...
      - run: go build -o pkgxray-guard ./examples/pkgxray-guard
      - run: ./pkgxray-guard recheck package-lock.json   # exit 2/3 fails the build
```
